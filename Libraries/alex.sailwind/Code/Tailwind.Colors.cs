...
partial class TailwindPanelComponent
{
	...
	private void GenerateColorUtilities( StringBuilder sb )
	{
		foreach ( var (name, value) in colors )
		{
			...
			GenerateUtility( sb, $"text-{name}", $"color: {value}", includeHover: true );
			GenerateUtility( sb, $"bg-{name}", $"background-color: {value}", includeHover: true );
			GenerateUtility( sb, $"border-{name}", $"border-color: {value}", includeHover: true );
		}
		...
		foreach ( var className in customClasses )
		{
			...
			GenerateUtility( sb, $"bg-[{color}]", $"background-color: {color}", includeHover: true );
		}
	}
}
